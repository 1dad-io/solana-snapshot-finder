...
- If a full snapshot has already been downloaded locally for a candidate base slot, later candidates with the same full slot will reuse that local full snapshot and retry only compatible incrementals instead of re-downloading the same full archive.
