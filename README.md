...
When a local full snapshot already exists, the tool checks whether it is still fresh enough to reuse. If it is reusable, the tool treats that full snapshot as the recovery base and searches only for compatible incremental snapshots built on the same full snapshot slot. If no compatible incremental is currently available, the tool keeps the reusable local full snapshot and exits cleanly by default. If `--allow-full-snapshot-fallback` is enabled, it may fall back to full snapshot discovery instead. If no reusable local full snapshot exists, the tool downloads a full snapshot and then proactively searches for a matching recent incremental snapshot, even if the originally selected candidate did not include one. Incremental-only recovery is activated only for a reusable local full snapshot or for a full snapshot downloaded during the current search flow; stale non-reusable local full snapshots do not block discovery of newer full snapshots. Snapshot selection is handled snapshot-set-first, so peers serving the same full+incremental pair are exhausted together before the tool returns to outer discovery, which avoids downloading a second full snapshot in the same pass after a dead incremental pair collapses. The global `--newer-snapshot-timeout` budget is now also enforced inside snapshot-set retries and replacement-incremental rescans, so one dead set cannot consume the whole run unchecked. After a full snapshot has already finished downloading in the current flow, the tool still allows the immediate incremental recovery attempt for that base instead of invalidating the just-completed full download retroactively. That same grace also applies to retrying more peers from the already-selected snapshot set, but it does not permit switching to a different snapshot set once the global budget is exhausted. A freshly downloaded full snapshot is treated as a successful final result only when it is standalone-usable under `--maximum-local-snapshot-age` or when the tool also obtains a compatible incremental snapshot.
...
- If a full snapshot has already been downloaded locally during the current search flow, the tool switches into incremental-only recovery for that base slot and avoids downloading newer full snapshots during the same search flow.
- Peers advertising the same full+incremental snapshot set are now handled together, so the tool exhausts that set before returning to outer discovery instead of hopping between equivalent dead pairs.
- A stale non-reusable local full snapshot found on disk does not by itself activate incremental-only recovery.
...
- The newer-snapshot budget is enforced not only between outer search iterations but also while retrying peers inside one snapshot set and while rescanning for replacement incrementals.
- If a full snapshot already finished downloading in the current flow, the tool still allows the immediate incremental or replacement-incremental recovery attempt for that downloaded base even if the global newer-snapshot budget has just been exceeded.
- Once that grace is active, the tool may continue retrying peers from the same already-selected snapshot set, but it still will not advance to a different snapshot set after the global budget is exhausted.
